...
// Auto-post when a vendor bill is recorded (DR Expense, CR AP)
export async function postBillRecorded(orgId, bill) {
  try {
    await connectDB()
    const [apAcc, expAcc] = await Promise.all([
      getAccount(orgId, CODES.accountsPayable),
      getAccount(orgId, '5020') || getAccount(orgId, '5010'), // operating expense fallback
    ])
    if (!apAcc || !expAcc) return null
    return await postJournalEntry(orgId, {
      date: bill.billDate || new Date(),
      narration: `Bill recorded: ${bill.billNumber} — ${bill.vendor?.name}`,
      reference: bill.billNumber,
      sourceType: 'Bill',
      sourceId: bill._id,
      currency: bill.currency || 'INR',
      lines: [
        { accountId: expAcc._id, accountCode: expAcc.code, accountName: expAcc.name, debit: bill.total, credit: 0,         narration: 'Expense' },
        { accountId: apAcc._id,  accountCode: apAcc.code,  accountName: apAcc.name,  debit: 0,          credit: bill.total, narration: 'Accounts Payable' },
      ],
    })
  } catch (e) { console.error('postBillRecorded error:', e.message); return null }
}
