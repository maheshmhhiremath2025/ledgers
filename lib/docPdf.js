...
export async function generateDocPdf(doc, cfg = {}) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const tbx = L + CW - 230
      pdf.rect(tbx, y, 230, 70).stroke(LINE)
      pdf.font('Regular').fontSize(9).fillColor(MUTED).text('Subtotal', tbx + 12, y + 10)
      pdf.font('Regular').fontSize(9).fillColor(TEXT).text(fmt(doc.subtotal), tbx + 12, y + 10, { width: 206, align: 'right' })
      pdf.font('Regular').fontSize(9).fillColor(MUTED).text('Tax',      tbx + 12, y + 26)
      pdf.font('Regular').fontSize(9).fillColor(TEXT).text(fmt(doc.taxTotal), tbx + 12, y + 26, { width: 206, align: 'right' })
      pdf.moveTo(tbx + 10, y + 44).lineTo(tbx + 220, y + 44).stroke(LINE)
      pdf.font('Bold').fontSize(11).fillColor(ACCENT).text('Total', tbx + 12, y + 50)
      pdf.font('Bold').fontSize(11).fillColor(ACCENT).text(fmt(doc.total), tbx + 12, y + 50, { width: 206, align: 'right' })
      y += 82
