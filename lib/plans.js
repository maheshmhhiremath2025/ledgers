...
// Auto-downgrade expired paid plans to starter. Returns true if downgrade happened.
export async function downgradeIfExpired(user) {
...
export async function withPlan(req, res) {
  ...
  if (user.role !== 'admin') {
    ...
    if (orgAdmin) {
      await downgradeIfExpired(orgAdmin)
      resetIfNewPeriod(orgAdmin)
      return orgAdmin
    }
  }

  await downgradeIfExpired(user)
  resetIfNewPeriod(user)
  return user
}