...
export default async function handler(req, res) {
  ...
  if (req.method === 'PUT') {
    try {
      ...
      if (!est) return res.status(404).json({ error: 'Quotation not found' })
      audit(req, auth, { action: 'quotation.update', entityType: 'Quotation', entityId: est._id, entityRef: est.estimateNumber, amount: est.total })
      return res.status(200).json(est)
  ...
  if (req.method === 'DELETE') {
    ...
    if (!est) return res.status(404).json({ error: 'Quotation not found' })
    audit(req, auth, { action: 'quotation.delete', entityType: 'Quotation', entityId: est._id, entityRef: est.estimateNumber, before: est.toObject() })
