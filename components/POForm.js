...
export default function POForm({ org, headers, toast, editItem, onClose }) {
  ...
  const vendState = (vendGstin || '').slice(0, 2)
  const isInter   = orgState && vendState && orgState !== vendState
        ...
        <div style={{ padding: '14px 18px', borderTop: '1px solid var(--border)', background: 'var(--bg-3)', display: 'flex', justifyContent: 'flex-end' }}>
          <div style={{ width: 300 }}>
            ...
            <div style={{ marginTop:6,fontSize:10,color:'var(--text-4)',textAlign:'right' }}>
              {isInter ? `Inter-state · IGST (${orgState} → ${vendState})` : orgState && vendState ? `Intra-state · CGST + SGST` : 'Defaulting to CGST + SGST'}
            </div>
          </div>
        </div>
      </Card>
